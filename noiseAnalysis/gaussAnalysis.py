        ...
        if generateNoise:
            ...
            if useMP and (iterations > 100):
                ...
                if min(maxProcess, mp.cpu_count()) > 1:
                    print(f"generating gauss noise on {min(maxProcess, mp.cpu_count())} cores")
                else:
                    print("generating gauss noise on 1 core")
            ...
            for ii, k in enumerate((kmean, kstd, kskewness, kkurtosis)):
                ...
                np.save(f'{directory}/gauss_noise_{names[ii]}.dat', k)
            ...
            try:
                kmean = np.load(f'{directory}/gauss_noise_kmean.dat.npy')
                kstd = np.load(f'{directory}/gauss_noise_kstd.dat.npy')
                kskewness = np.load(f'{directory}/gauss_noise_kskewness.dat.npy')
                kkurtosis = np.load(f'{directory}/gauss_noise_kkurtosis.dat.npy')
